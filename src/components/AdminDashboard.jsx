...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (activitiesRes.data?.success) setRecentActivities(activitiesRes.data.orders || []);
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <StatCard title="Total Orders" value={stats.totalOrders} icon={<ShoppingCart className="h-10 w-10 text-blue-600" />} />
        <StatCard title="Total Revenue" value={`$${safeMoney(stats.totalRevenue)}`} icon={<AttachMoney className="h-10 w-10 text-green-600" />} />
        <StatCard title="Total Users" value={stats.totalUsers} icon={<People className="h-10 w-10 text-purple-600" />} />
        <StatCard title="Pending Orders" value={stats.pendingOrders} icon={<Receipt className="h-10 w-10 text-yellow-600" />} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          <div className="bg-white border rounded-xl">
            {recentActivities.length > 0 ? recentActivities.map(order => (
              <div key={order._id} className="p-4 border-b last:border-0">
        ...
        <div>
          ...
          <div className="bg-white border rounded-xl">
            {recentUsers.length > 0 ? recentUsers.map(user => (
              <div key={user._id} className="p-4 border-b last:border-0">
                <p className="font-medium">{user.name || "New User"}</p>
                <p className="text-xs text-gray-500">{formatTime(user.createdAt)}</p>
              </div>
            )) : (
